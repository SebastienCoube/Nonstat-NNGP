...
seeds = seq(30)
tau_design = as.matrix(expand.grid(matern_range_vals,seeds))
saveRDS(tau_design, "tau_design.RDS")

for(i in seq(nrow(tau_design))){
  if (!(paste("tau_exp_started_", i, sep = "") %in% list.files())){
    saveRDS(1, paste("tau_exp_started_", i, sep = ""))
  seed = tau_design[i,2]
  matern_range = tau_design[i,1]
  set.seed(seed)
  # spatial locations
  locs = cbind(runif(11000), runif(11000))
  # noiseless GP 
  log_range = (GpGp::fast_Gp_sim(c(0.000001,.2, 1.5, 0), locs = rbind(locs), m=10) -  4)
  # latent field
  ...
  tau_var = 1
  tau_range = .1
  tau = exp(GpGp::fast_Gp_sim(c(tau_var,tau_range, 1.5, 0), locs = rbind(locs), m=10) - 1)
  ...
  mcmc_nngp_list = Bidart::mcmc_nngp_initialize_nonstationary(
    ...
    noise_PP = T, PP = PP, noise_log_scale_prior = c(-10, 2.5)
  )
  ...
  res = c(
    ...
    log_tau_scale =  estimation$summaries$noise_log_scale[1], 
  ...
  saveRDS(res, paste("tau_exp_ended_", i, sep = ""))
}}
