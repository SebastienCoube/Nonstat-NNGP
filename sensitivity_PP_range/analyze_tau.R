...
res = as.data.frame(t(sapply(list.files()[grep("tau_exp_ended", list.files())], readRDS )))
input = readRDS("tau_design.RDS")

log_r = log(input[sapply(row.names(res), function(x)as.numeric(substr(x, 15, nchar(x)))),1])
plot(log_r, res$w_smooth_MSE)
plot(log_r, res$w_pred_MSE)
plot(log_r, res$log_tau_MSE)
plot(log_r, res$log_tau_scale)


boxplot(res$log_tau_scale ~ round(log_r, 3))
boxplot(res$log_tau_MSE ~ round(log_r, 3))
...
out_ = signif(cbind(
  summary(lm(res$log_tau_scale ~log_r_factor + as.factor(res$seed.Var2)))$coefficients[seq(5), c(1, 4)],
  summary(lm(res$log_tau_MSE ~log_r_factor + as.factor(res$seed.Var2)))$coefficients[seq(5), c(1, 4)],
