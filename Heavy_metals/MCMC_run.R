...
if(length(grep("validation_train.RDS", list.files("Heavy_metals/")))==0)
{
  ...
  # get KL basis
  train_KL_decomposition      = Bidart::get_KL_basis(train_locs, lonlat = T, covparms = c(1, 3, 1, 0), m = 8)
  # save
...
mcmc_nngp_list = Bidart::mcmc_nngp_initialize_nonstationary (
  ...
  m = 8, #number of Nearest Neighbors
  reordering = c("maxmin"), #Reordering
  covfun = "exponential_sphere", response_model = "Gaussian", # covariance model and response model
  noise_X = NULL, # range for latent field of parameters, if NULL no latent field
  scale_X = NULL, # range for latent field of parameters, if NULL no latent field
...
mcmc_nngp_list = Bidart::mcmc_nngp_initialize_nonstationary (
  ...
  noise_X = as.data.frame(cbind(train_data_set$train_KL_decomposition$basis[,seq(30)], train_data_set$train_X)), # range for latent field of parameters, if NULL no latent field
  range_X = as.data.frame(cbind(train_data_set$train_KL_decomposition$basis[,seq(30)], train_data_set$train_X[, c("gcarb", "globedem", "twi")])), # range for latent field of parameters, if NULL no latent field
  n_chains = 3,  # number of MCMC chains
  seed = 2
)
remove(train_data_set)
