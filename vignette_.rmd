...
This subsection treats how the possibly **nonstationary correlation** is modeled. This is the biggest lump. 
...
In both case, a **logarithmic parametrization** (or matrix-logarithmic parametrization in the case of range ellipses) is used: $$log(\alpha(s)) = x_\alpha(s)\beta_\alpha$$ in the scalar case, $\alpha$ being the range parameter. This is the middle doll. We use $$vech(logM(A(s))) = x_A(s)\beta_A$$ in the matrix case, $A$ being the (matrix) range parameter, $vech$ being the [half-vectorization](https://en.wikipedia.org/wiki/Vectorization_(mathematics)#Half-vectorization) and $logM$ being the [matrix logarithm](https://en.wikipedia.org/wiki/Logarithm_of_a_matrix).
Here, $\beta_A$ is a **matrix with** $3$ columns. This is the left-hand doll. 
Note that the range parameters $\alpha(s)$ or $A(s)$ are **local**, they depend on the **spatial site $s$**. Therefore, they enforce a **local behavior of the Gaussian process** w(s). 
...
Note that $A(s)$ is parametrized using **matrix logarithm**. What does it imply ? 
...
If only the first component is allowed to move and the two other are set to $0$, we get a [**nonstationary, locally isotropic**](#locally_iso) model.
...
img = (png::readPNG("NDVI_r2/maps/observed.png"))
plot(1,1)
grid::grid.raster(img)
...
```{r,fig.width = 9, fig.height=9, echo=FALSE}
img = (png::readPNG("NDVI_r2/maps/ellipses.png"))
plot(1,1)
grid::grid.raster(img)
...
```{r,fig.width = 9, fig.height=9}
img = (png::readPNG("NDVI_r2/maps/noise.png"))
plot(1,1)
grid::grid.raster(img)
...
```{r,fig.width = 9, fig.height=9}
img = (png::readPNG("NDVI_r2/maps/log_sd.png"))
plot(1,1)
grid::grid.raster(img)
...
```{r,fig.width = 9, fig.height=9}
img = (png::readPNG("NDVI_r2/range_var.png"))
plot(1,1)
grid::grid.raster(img)
```
