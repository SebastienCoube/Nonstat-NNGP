...
###########################################
# nonstationary spatially coherent effect #
###########################################
# spatial basis functions
PP_INLA = Bidart::get_PP(observed_locs = prmesh$loc[,-3], matern_range = .2, 
                         n_PP = 50)
...
Bidart::plot_pointillist_painting(prmesh$loc[,-3], PP_basis[,1])
Bidart::plot_pointillist_painting(prmesh$loc[,-3], PP_basis[,2])

nu = 1
B.kappa = as.matrix(cbind(0, 0, 1, PP_basis))
B.tau =   as.matrix(cbind(0, 1, 0, -nu*PP_basis) )
spde_spatially_coherent <- inla.spde2.matern(
  prmesh, 
  B.tau = B.tau, 
  B.kappa = B.kappa, 
  theta.prior.mean = rep(0, ncol(B.kappa) - 1), 
  theta.prior.prec = rep(.01, ncol(B.kappa) - 1), 
  alpha = nu+1
)
s.index = INLA::inla.spde.make.index(
  name = "spatial.field",
...
saveRDS(res, "runINLA_heterosk_range.RDS")
...
pdf("INLA_heterosk_var_range.pdf")
...
saveRDS(score, "inla_heterosk_range_score.RDS")
