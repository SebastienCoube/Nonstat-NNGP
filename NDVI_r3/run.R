...
##cheatsheet to load results fromm cluster on laptop
#setwd("NDVI_r3")
#tatato = readRDS("res/NNGP_full_monty.RDS")
#names(tatato) = c(  
# "mcmc_nngp_list",
# "preds_field",
# "preds_fixed",
...
if(length(grep(paste(runs_design[i_model,1], "_lock", sep=""), list.files()))==0){
  ...
  # Coverage and MSE #
  ####################
  estimated_mean_samples = estimated_fixed$predicted_samples + estimated_parameters$samples$field[mcmc_nngp_list$vecchia_approx$locs_match,,,drop = F]
  estimated_noisevar_samples = exp(estimation_noise$predicted_samples) 
  response_train_samples = (estimated_mean_samples + sqrt(estimated_noisevar_samples) * rnorm(length(estimated_noisevar_samples)))[,1,]
  quantiles_train = apply(response_train_samples, 1, function(x)quantile(x, probs = c(.025, .975)))
  train_coverage = mean((train_field> quantiles_train[1,])&(train_field<quantiles_train[2,]))
  
  preds_mean_samples = preds_fixed$predicted_samples + preds_field$predicted_samples$field
  preds_noisevar_samples = exp(preds_noise$predicted_samples) 
  ...
  loo_coverage = mean((response_variable[loo_idx_expanded] > quantiles_preds[1,-seq(nrow(lump_center_locs))])&(response_variable[loo_idx_expanded] <quantiles_preds[2,-seq(nrow(lump_center_locs))]))
  lump_center_coverage = mean((response_variable[lump_center_idx_expanded] > quantiles_preds[1,seq(nrow(lump_center_locs))])&(response_variable[lump_center_idx_expanded] <quantiles_preds[2,seq(nrow(lump_center_locs))]))
  
  loo_MSE = mean((response_variable[loo_idx_expanded] - apply(preds_fixed$predicted_samples + preds_field$predicted_samples$field, 1, mean) [-seq(nrow(lump_center_locs))])^2)
  lump_center_MSE = mean((response_variable[lump_center_idx_expanded] - apply(preds_fixed$predicted_samples + preds_field$predicted_samples$field, 1, mean) [seq(nrow(lump_center_locs))])^2)
  ...
  scores = c(
    "train_elpd" = Bidart::log_score_Gaussian(
      observed_field = train_field,
    ...
    "loo_elpd" = Bidart::log_score_Gaussian(
    ...
    "lump_elpd" = Bidart::log_score_Gaussian(
    ...
    "lump_center_coverage" = lump_center_coverage, 
    "loo_coverage" = loo_coverage, 
    "loo_MSE" = loo_MSE, 
    "lump_center_MSE" = lump_center_MSE, 
    "time" = Sys.time() - t1
  )
