...
if(length(grep("validation_train.RDS", list.files("Heavy_metals_comparison/")))==0)
{
  ...
  # get KL basis
  train_KL_decomposition      = Bidart::get_KL_basis(train_locs, lonlat = T, covparms = c(1, 3, 1, 0), m = 10, n_KL = 100)
  # save
...
##################################################
# nonstationary noise, scale and range matern 15 #
##################################################
...
mcmc_nngp_list = Bidart::mcmc_nngp_initialize_nonstationary (
  ...
  m = 10, #number of Nearest Neighbors
  reordering = c("maxmin"), #Reordering
  covfun = "nonstationary_matern_isotropic_sphere", response_model = "Gaussian", # covariance model and response model
  ...
  scale_X = as.data.frame(cbind(train_data_set$train_KL_decomposition$basis[,seq(25)], train_data_set$train_X[, c("gcarb", "globedem", "twi")])), # range for latent field of parameters, if NULL no latent field
  n_chains = 3,  # number of MCMC chains
  seed = 1
)
...
mcmc_nngp_list = Bidart::mcmc_nngp_initialize_nonstationary (
  ...
  m = 10, #number of Nearest Neighbors
  reordering = c("maxmin"), #Reordering
  covfun = "matern_sphere", response_model = "Gaussian", # covariance model and response model
  nu = 1.5,
  noise_X = as.data.frame(cbind(train_data_set$train_KL_decomposition$basis[,seq(25)], train_data_set$train_X)), # range for latent field of parameters, if NULL no latent field
  scale_X = NULL, # range for latent field of parameters, if NULL no latent field
  range_X = NULL, # range for latent field of parameters, if NULL no latent field
...
############################################
# nonstationary noise, scale and range exp #
############################################
train_data_set = readRDS("Heavy_metals_comparison/validation_train.RDS")
mcmc_nngp_list = Bidart::mcmc_nngp_initialize_nonstationary (
  observed_locs  = train_data_set$train_locs, #spatial locations
...
#####################################
# nonstationary noise and range exp #
#####################################
train_data_set = readRDS("Heavy_metals_comparison/validation_train.RDS")
mcmc_nngp_list = Bidart::mcmc_nngp_initialize_nonstationary (
  observed_locs  = train_data_set$train_locs, #spatial locations
