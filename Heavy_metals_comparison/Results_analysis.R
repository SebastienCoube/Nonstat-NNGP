...
gridmaps <- rgdal::readGDAL("Heavy_metals_comparison/usgrids5km/dairp.asc")
names(gridmaps)[1] <- sub(".asc", "", grid.list[1])
for(i in grid.list[-1]) {
  gridmaps@data[sub(".asc", "", i[1])] <- rgdal::readGDAL(paste("Heavy_metals_comparison/usgrids5km/",i, sep = ""))$band1
}
# projection coords
...
# normalizing X_pred
X_mean = readRDS("Heavy_metals_comparison/processed_data.RDS")$X_locs_mean
X_sd = readRDS("Heavy_metals_comparison/processed_data.RDS")$X_locs_sd
...
sp::plot(usa_aea, add = T)
dev.off()
pdf("Heavy_metals_comparison/field_mean_scale.pdf", width = 2, height = 5)
sp::plot(gridmaps["field_stat"], what = "scale", zlim = zlim )
dev.off()
...
pdf("Heavy_metals_comparison/noise_all.pdf", height = 3, width = 12)
layout(matrix(1:4, 1, 4), widths = c(5,5,5,1))
zlim = c(
