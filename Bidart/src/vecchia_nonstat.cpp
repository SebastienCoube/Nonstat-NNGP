      ...
      if(compute_derivative)
      {
        ...
        for(int j1=1; j1<bsize; j1++){
          for(int j2=1; j2<bsize; j2++){
            ...
            dsigma11 (j1-1, j2-1) = (
              pow(1.00001 * range(NNarray(i,j1) - 1) *    range(NNarray(i,j2) -1)    , dim*.25  ) * 
                pow(1.00001 * range(NNarray(i,j1) - 1)*.5 + range(NNarray(i,j2) -1)*.5 , dim*(-.5)) * 
                exp(- pow(sqeucdist(j1, j2)/ (1.00001 * range(NNarray(i,j1) - 1)*.5 + range(NNarray(i,j2) -1)*.5), .5))
            - sigma11 (j1-1, j2-1)) * 100000 
            ;
          }
          ...
          dsigma12_parents (j1-1) = (
            pow(1.00001 * range(NNarray(i,j1) - 1)    * range(NNarray(i,0) - 1)    , dim*  .25) * 
              pow(1.00001 * range(NNarray(i,j1) - 1)*.5 + range(NNarray(i,0) - 1)*.5 , dim*(-.5)) * 
              exp(-pow(sqeucdist(j1, 0)/ (1.00001 * range(NNarray(i,j1) - 1)*.5 + range(NNarray(i,0) - 1)*.5), .5)) 
            - sigma12(j1-1))  * 100000 ;
          //The derivative  of sigma 12 wrt to child range is a dense vector 
          dsigma12_child (j1-1) = (
            pow(range(NNarray(i,j1) - 1)    * 1.00001 * range(NNarray(i,0) - 1)    , dim*  .25) * 
              pow(range(NNarray(i,j1) - 1)*.5 + 1.00001 * range(NNarray(i,0) - 1)*.5 , dim*(-.5)) * 
              exp(-pow(sqeucdist(j1, 0)/ (range(NNarray(i,j1) - 1)*.5 + 1.00001 * range(NNarray(i,0) - 1)*.5), .5)) 
            - sigma12(j1-1))  * 100000 ;
        }
  ...
  if(covfun_name == "nonstationary_exponential_anisotropic")
  {
    if(log_range.n_cols==3)
    {
      ...
      for(int i=0; i<n; i++){
        ...
        logmat(0, 0) = logmat(0, 0) + 0.00001 ; 
        d1range_matrices.slice(i) = expmat_sym(logmat) ; 
        
        logmat(0, 0) = logmat(0, 0) - 0.00001 ; 
        logmat(1, 1) = logmat(1, 1) + 0.00001 ; 
        d2range_matrices.slice(i) = expmat_sym(logmat) ; 
        
        logmat(1, 1) = logmat(1, 1) - 0.00001 ; 
        logmat(0, 1) = logmat(0, 1) + 0.00001 * pow(2, -.5) ; 
      ...
      for(int i=1; i<n; i++){
        ...
        d1sigma11 = (d1sigma11 - sigma11)/0.00001;
        d2sigma11 = (d2sigma11 - sigma11)/0.00001;
        d3sigma11 = (d3sigma11 - sigma11)/0.00001;
        d1sigma12_parents = (d1sigma12_parents - sigma12)/0.00001;
        d2sigma12_parents = (d2sigma12_parents - sigma12)/0.00001;
        d3sigma12_parents = (d3sigma12_parents - sigma12)/0.00001;
        d1sigma12_child   = (d1sigma12_child   - sigma12)/0.00001;
        d2sigma12_child   = (d2sigma12_child   - sigma12)/0.00001;
        d3sigma12_child   = (d3sigma12_child   - sigma12)/0.00001;
