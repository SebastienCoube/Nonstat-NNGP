---
title: "Toy example range locally isotropic"
output: html_document
---

This toy example shows how to model *locally isotropic range*.
It shows : 

- how to initialize the model
- how to diagnose convergence
- how to compare the models (with a stationary model) using DIC
- how to detect over-modelling with a locally anisotropic range, or with locally isotropic range and nonstationary variance
...
Check convergence, look at the Gelman-Rubin-Broks diagnostics (they are low) 
```{r, dpi = 50}
...
Convergence is checked with diagnostic_plots() like before (not done here to save some html size)
...
Lo and behold, the two ``anisotropy" components are low ! (normally this pops up using Bidart::mcmc_nngp_run_nonstationary() )
```{r, dpi = 50}
...
Lo and behold, the marginal variance's log scale is low ! (those plots pop up)
```{r, dpi = 50}
Bidart::plot_log_scale(log_scale_arrays =
                         list(
