...
Bidart::plot_pointillist_painting(prmesh$loc, PP_basis_prmesh[,1], cex = .1)
...
spde_spatially_coherent = INLA::inla.spde2.matern(mesh = prmesh, alpha=2,
                        B.tau = cbind(rep(1, nrow(PP_basis_prmesh))),
                        B.kappa = cbind(1, PP_basis_prmesh)# Nonstationarity here
)
...
saveRDS(res, "runINLA_nonstat_range.RDS")
